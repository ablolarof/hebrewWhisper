...
| **The STOP box prints** | Confirmed by eye 2026-08-01. `importlib.invalidate_caches()` is sufficient to see the new version from the same process. This was the last structural unknown, and the mechanism the shareability story depends on: a recipient with no chat transcript now gets told to restart |
...
| That `vtt`, `tsv` and `json` load correctly in their **target applications** | Structure is unit-tested, but nobody has opened a `.vtt` in a player, a `.tsv` in Excel, or parsed the `.json` elsewhere. |
